...
- End-to-end learning (perception → action)
...
```mermaid
graph LR
    Agent[Agent<br/>Policy π] -->|Action a_t| Env[Environment<br/>Robot + World]
    Env -->|State s_t| Agent
...
1. Collect dataset D = \{(s₁, a₁), (s₂, a₂), ..., (sₙ, aₙ)\} from expert
...
**Algorithm:**
1. Initialize dataset D ← expert demonstrations
2. For N iterations:
   - Train policy π on D
...
**Key insight**: Policy visits its own states, expert provides corrections → no distribution shift
...
**Reality gap** → Policies fail when deployed on real robots
...
```python
state → [FC → ReLU → FC → ReLU → FC] → action
```
...
For **vision-based control** (raw pixels → actions):

```python
image → [Conv → ReLU → Pool → Conv → ReLU → Pool → FC] → action
```
...
```python
observation_t → [LSTM] → hidden_state_t → action_t
```
...
graph TB
    subgraph "Training (Simulation)"
        ...
        Policy[Neural Network Policy<br/>Observations → Actions]
...
<ExerciseComponent
  exercise={{
    ...
    instructions: `
Implement Q-learning for a 3×3 grid world:
...
print(f"Logits: z₁={z1:.2f}, z₂={z2:.2f}")
