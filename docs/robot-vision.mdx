...
flowchart LR
    ...
    A[Input Image<br/>640×640] --> B[Backbone<br/>Feature Extractor]
...
4. Single forward pass → extremely fast (60+ FPS)
...
class StereoDepthEstimator:
    ...
    def estimate_depth(self, left_img, right_img):
        """Complete pipeline: images → depth map"""
        # Compute disparity
...
flowchart TB
    ...
    subgraph "Encoder (Downsampling)"
        I[Input Image<br/>512×512×3] --> E1[Conv Block 1<br/>256×256×64]
        E1 --> E2[Conv Block 2<br/>128×128×128]
        E2 --> E3[Conv Block 3<br/>64×64×256]
        E3 --> E4[Conv Block 4<br/>32×32×512]
    end

    subgraph "Bottleneck"
        E4 --> B[Conv Block 5<br/>16×16×1024]
    end

    subgraph "Decoder (Upsampling)"
        B --> D1[Upconv + Skip<br/>32×32×512]
        E4 -. Skip .-> D1
        D1 --> D2[Upconv + Skip<br/>64×64×256]
        E3 -. Skip .-> D2
        D2 --> D3[Upconv + Skip<br/>128×128×128]
        E2 -. Skip .-> D3
        D3 --> D4[Upconv + Skip<br/>256×256×64]
        E1 -. Skip .-> D4
    end

    subgraph "Output"
        D4 --> O[Segmentation Map<br/>512×512×num_classes]
    end
...
- Grid size: 20×20 (each cell is 32×32 pixels)
...
print(f"Dimensions: {box_width}×{box_height}")
...
2. Calculate depth (Z) using Z = (f × B) / d
...
`,
    ...
    hints: [
      ...
      'Doubling disparity → half the depth'
    ],
    solution: `
import numpy as np
...
# Verification
print(f"\\nVerification: disparity × depth = {disparity * depth:.0f} = f × B = {focal_length * baseline:.0f}")
...
# Depth: 9.00 meters
#
# Verification: disparity × depth = 90 = f × B = 90
`,
    difficulty: 'easy',
    estimatedTime: 15
  }}
...
<QuizComponent
  questions={[
    ...
    },
    {
      ...
      correctAnswer: 'option-b',
      explanation: 'Disparity is the horizontal pixel offset of the same point between stereo images. It is inversely proportional to depth: larger disparity means closer objects, smaller disparity means farther objects. Formula: Z = (f × B) / d.'
    },
    {
      id: 'vision-q3',
...
- **Stereo vision** computes depth through disparity: Z = (f × B) / d
